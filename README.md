...
**Current release: [Shorts Studio v0.7.3](https://github.com/wiifhub/AI-Youtube-Shorts-Generator/releases/tag/v0.7.3).** It adds a browser-free native desktop window, first-run diagnostics, recoverable project management, cached previews and waveforms, resumable jobs, caption-file exports, safer Windows packaging, the original Shorts Studio cut-ribbon logo, and transparent rounded-corner icons with explicit paths for reliable Windows shortcuts.
...
Download the current [ShortsStudio-v0.7.3-windows.zip](https://github.com/wiifhub/AI-Youtube-Shorts-Generator/releases/download/v0.7.3/ShortsStudio-v0.7.3-windows.zip) and extract the entire ZIP. Open the `ShortsStudio` folder and double-click `unblock_and_start.bat`; it removes the download block from the extracted files and starts `ShortsStudio.exe` in its native window. You can also launch the executable directly after choosing **More info > Run anyway** once. Keep the whole folder together because it includes the runtime, WebView2 shell dependencies, CTranslate2, optional CUDA 12 runtime libraries, FFmpeg binaries, and the branded rounded-corner shortcut icon. Add a `.env` file beside the executable with OpenAI or Gemini credentials for AI ranking; otherwise Local mode automatically uses offline transcript ranking. Use **Quit** in the page or tray to stop the server cleanly.

### Installed Windows application

For a normal Windows installation, download [ShortsStudio-Setup-v0.7.3.exe](https://github.com/wiifhub/AI-Youtube-Shorts-Generator/releases/download/v0.7.3/ShortsStudio-Setup-v0.7.3.exe). The installer adds a Start Menu entry, offers a Desktop shortcut with the branded rounded-corner icon, installs the bundled runtime and FFmpeg, and registers a normal uninstaller. The app runs locally in its native window; no cloud account is required for the UI. If the preferred port is already in use, the launcher selects another loopback port automatically.
